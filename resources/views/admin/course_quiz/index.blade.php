                  ...
                  @if($quiz_question->type == 'radio' || $quiz_question->type == 'checkbox')
                    <div class="col-md-6 col-xl-3 webdesign illustrator">
                        <div class="gal-detail card p-2">
                            <div>
                                <h5 class="float-left font-16 mt-3">Question {{ $key+1 }}:
...
@push('script')
    ...
    $(document).on('click', '#convertGapQuestion', function(e) {
        e.preventDefault();
        let quiz_gap_question  = $('textarea.gap_text_question').val();
        // const searchRegExp     = /\([^()]+\)/g;
        const searchRegExp     = /\{\{.+?\}\}/g;
        var converted_question = quiz_gap_question.replace(searchRegExp,'____');
        // var converted_question_with_input_tag = quiz_gap_question.replace(searchRegExp,"<input type="+"'text'"+" value="+"''"+" name="+"'gap_ans_list[]'"+"/>");
        // console.log(converted_question_with_input_tag);
        // console.log(quiz_gap_question.replace(searchRegExp, '___'));
        // var converted_question_html = quiz_gap_question.replace(searchRegExp,'<input name="gap_quiz[]"/>');
        $("textarea#conversion_gap_text").val(converted_question);
        // $("textarea#conversion_gap_text_with_input").val(converted_question_with_input_tag);

        var ans_array = quiz_gap_question.match(searchRegExp);
        let length_ans_array = ans_array.length;
        var gap_input_array = "";
        for(let i = 0; i<length_ans_array; i++){
            var ans =  ans_array[i].slice(2, -2);
            gap_input_array+='<input name="gap_answers['+i+']'+'" type="hidden" value="'+ans+'"/>';
        }
        $('.gap_answer_div').empty().append(gap_input_array);
        console.log(gap_input_array);
    });
